...
fn daa(r: &mut z80::Registers) {
    ...
    let d = z80::DAA_TABLE[idx as usize];
    r.a = (d >> 8) as u8;
    r.f = d as u8;
}
...
pub fn exec(inst: u8, r: &mut z80::Registers, m: &mut mem::Memory) -> u32 {
...
pub fn exec_cb(inst: u8, r: &mut z80::Registers, m: &mut mem::Memory) -> u32 {
    macro_rules! rl( ($reg:expr, $cy:expr) => ({
        let ci = if (r.f & C) != 0 {1} else {0};
        let co = $reg & 0x80;
        $reg = ($reg << 1) | ci;
