...
JBA.Input.prototype = {
  ...
  reset: function() {
    ...
    this.directions = 0xf;
    this.joypad_sel = 0;
    for (i = 0; i < this.sgbram.length; i++) {
      this.sgbram[i] = 0;
    }
    for (i = 0; i < 4; i++) {
  ...
  wb: function(addr, value) {
    ...
    switch (this.state) {
      ...
      case SGB.NONE:
        if (bits == 0) {
          this.state = SGB.RESET;
          this.sgb_packets = 0;
        } else if (bits == 3) {
          this.joypad_sel = (this.joypad_sel + 1) % 4;
        }
        break;
  ...
  mask_en: function() {
    ...
    if (b == 0) {
      this.memory.gpu.lcdon = 1;
    } else if (b == 1) {
      this.memory.gpu.lcdon = 0;
    } else if (b == 2) {
      this.memory.gpu.white_canvas();
    } else if (b == 3) {
      this.memory.gpu.white_canvas();
    }
  },
